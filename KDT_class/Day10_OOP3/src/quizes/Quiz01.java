...
public class Quiz01 {
    static String menuRetryMsg = "잘못된 입력입니다.\n다시 입력하세요.\n>> ";
    static String regIdRetryMsg = "잘못된 입력입니다.\n다시 입력하세요.\n신규 영화 ID: ";

    static Movie[] movies = new Movie[5];
    static int regMovieCnt = 0;
    // 현재 등록된 영화의 수
    ...
    public static void regMovie() {
        ...
        while(checkIdDuplicated(regId) || regId == -1) {
        ...
        movies[regMovieCnt++] = new Movie(regId, regTitle, regGenre);
        // 영화 등록
    }

    // 영화 ID 중복 확인
    public static boolean checkIdDuplicated(int id) {
        if(id < 0) {
            // id 값이 음수라면 false
